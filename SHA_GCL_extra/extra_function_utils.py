...
def generate_sample_rate_vector_sep(Dataset_choice, num_stage_predicate):
    if Dataset_choice == 'VG':
        predicate_new_order_count = [3024465, 109355, 67144, 47326, 31347, 21748, 15300, 10011, 11059, 10764, 6712,
                                     5086, 4810, 3757, 4260, 3167, 2273, 1829, 1603, 1413, 1225, 793, 809, 676, 352,
                                     663, 752, 565, 504, 644, 601, 551, 460, 394, 379, 397, 429, 364, 333, 299, 270,
    ...
    for i in range(len(num_stage_predicate)):
        ...
        for j in range(len(predicate_new_order_count)):
            opiece.append(0.0)
        if prev == 0:
            num_list = predicate_new_order_count[prev:(num_stage_predicate[i] + 1)]
        else:
            num_list = predicate_new_order_count[prev+1:(num_stage_predicate[i]+1)]
            num_list.insert(0, predicate_new_order_count[0])
        median = np.median(num_list[1:])
        for j in range(len(num_list)):
            if num_list[j] > median:
                num = median / num_list[j]
                ...
                opiece[j+prev] = 1.0
        outp.append(opiece)
        prev = num_stage_predicate[i]
    return outp

def generate_sample_rate_vector_sep2(Dataset_choice, num_stage_predicate):
    if Dataset_choice == 'VG':
        predicate_new_order_count = [3024465, 109355, 67144, 47326, 31347, 21748, 15300, 10011, 11059, 10764, 6712,
                                     5086, 4810, 3757, 4260, 3167, 2273, 1829, 1603, 1413, 1225, 793, 809, 676, 352,
                                     663, 752, 565, 504, 644, 601, 551, 460, 394, 379, 397, 429, 364, 333, 299, 270,
        ...
        predicate_new_order_count = [200000, 64218, 47205, 32126, 25203, 21104, 15890, 15676, 7688, 6966, 6596, 6044,
                                     5250, 4260, 4180, 4131, 2859, 2559, 2368, 2351, 2134, 1673, 1532, 1373, 1273, 1175,
                                     1139, 1123, 1077, 941, 916, 849, 835, 808, 782, 767, 628, 603, 569, 540, 494, 416,
                                     412, 412, 398, 395, 394, 390, 345, 327, 302, 301, 292, 275, 270, 267, 267, 264,
                                     258, 251, 233, 233, 229, 224, 215, 214, 209, 204, 198, 195, 192, 191, 185, 181,
                                     176, 158, 158, 154, 151, 148, 143, 136, 131, 130, 130, 128, 127, 125, 124, 124,
                                     121, 118, 112, 112, 106, 105, 104, 103, 102, 52, 52]
        assert len(predicate_new_order_count) == 101
    else:
        exit('wrong mode in Dataset_choice')
    outp = []
    ...
    for i in range(len(num_stage_predicate)):
        ...
        for j in range(len(predicate_new_order_count)):
            opiece.append(0.0)
        if prev == 0:
            num_list1 = predicate_new_order_count[prev:(num_stage_predicate[i] + 1)]
        else:
            num_list1 = predicate_new_order_count[prev + 1:(num_stage_predicate[i] + 1)]
            num_list1.insert(0, predicate_new_order_count[0])
        median = np.median(num_list1[1:])
        for j in range(len(num_list1)):
            if num_list1[j] > median:
                num = median / num_list1[j]
                if j == 0:
                    num = num * 10.0
                if num < 0.01:
                    num = 0.01
                ...
                opiece[j + prev] = 1.0
        if prev!=0:
            num_list2 = predicate_new_order_count[1:prev+1]
            for j in range(len(num_list2)):
                if num_list2[j] > median:
                    num = median / num_list2[j]
                    if num < 0.01:
                        num = 0.01
                    ...
                    opiece[j+1] = 1.0
        if (num_stage_predicate[i] + 1) < len(predicate_new_order_count):
            num_list3 = predicate_new_order_count[(num_stage_predicate[i] + 1)::]
            for j in range(len(num_list3)):
                if num_list3[j] > max(num_list1):
                    num = median / num_list3[j]
                    if j == 0:
                        num = num * 10.0
                    if num < 0.01:
                        num = 0.01

                    opiece[j + num_stage_predicate[i] + 1] = num
                else:
                    #opiece[j + prev] = 1.0
                    opiece[j + num_stage_predicate[i] + 1] = 1.0
