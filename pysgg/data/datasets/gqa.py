...
class GQADataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, dict_file, train_file, test_file, transforms=None, depth_img_dir=None,
        ...
        if self.split == 'train':
            self.filenames, self.img_info, self.gt_boxes, self.gt_classes, self.relationships = load_graphs(
                self.train_file, self.split, num_im, num_val_im, DEBUG=cfg.DEBUG)
            # -- Initialize depth related items

        else:
            self.filenames, self.img_info, self.gt_boxes, self.gt_classes, self.relationships = load_graphs(
                self.test_file, self.split, num_im, num_val_im, DEBUG=cfg.DEBUG)
            # -- Initialize depth related items
        if self.depth_img_dir:
            # -- Load depth images' filenames --
            self.depth_filenames = [x.split('.')[0] + '-depth_raw.png' for x in self.filenames]
    ...
    def __getitem__(self, index):
        ...
        if self.depth_filenames is not None:
            depth_img = Image.open(os.path.join(self.depth_img_dir, self.depth_filenames[index]))
        ...
        if self.depth_transforms is not None:
            depth_img, _ = self.depth_transforms(depth_img, target)
...
def load_graphs(data_json_file, split, num_im, num_val_im, DEBUG=False):
    ...
    if split == 'val' and DEBUG:
        output_filenames = output_filenames[:num_val_im]
        output_img_info = output_img_info[:num_val_im]
        output_boxes = output_boxes[:num_val_im]
        output_classes = output_classes[:num_val_im]
        output_relationships = output_relationships[:num_val_im]
    elif split == 'val':
