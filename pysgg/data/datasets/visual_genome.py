...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, depth_img_dir=None,  transforms=None,
                 depth_transforms=None, filter_empty_rels=True, num_im=-1, num_val_im=5000, check_img_file=False,
        ...
        if cfg.DATASETS.REORDER_FREQ_BASED:
        ...
        self.filenames, self.img_info = load_image_filenames(
            img_dir, image_file, self.check_img_file, debug_mode=cfg.DEBUG)  # length equals to split_mask
        ...
        if self.depth_img_dir:
            # -- Load depth images' filenames --
            self.depth_filenames, _ = load_image_filenames(depth_img_dir, image_file, self.check_img_file, is_depth=True, debug_mode=cfg.DEBUG)
    ...
    def __getitem__(self, index):
        ...
        if self.depth_filenames is not None:
            depth_img = Image.open(self.depth_filenames[index])
        ...
        if self.transforms is not None:
            if pre_compute_boxlist is not None:
                ...
                if self.depth_transforms is not None:
                    depth_img, all_boxes = self.depth_transforms(depth_img, all_boxes)
                ...
                if self.depth_transforms is not None:
                    depth_img, target = self.depth_transforms(depth_img, target)

        if self.depth_filenames is not None:
            return img, target, index, depth_img
        else:
...
def load_image_filenames(img_dir, image_file, check_img_file, is_depth=False, debug_mode=False):
    """
    ...
    for i, img in enumerate(im_data):
        if is_depth:
            basename = '{}-depth_raw.{}'.format(img['image_id'], fmt)  # for new depth_map
        ...
        if os.path.exists(filename) or not check_img_file:
            fns.append(filename)
            img_info.append(img)
        if debug_mode and len(fns)==2000:
            break
    if not debug_mode:
