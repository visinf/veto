...
def do_vg_evaluation(
    ...
    if "relations" in iou_types:
        ...
        evaluator = {}
        avg_metrics = None
        if cfg.ENSEMBLE_LEARNING.ENABLED and 'pred_rel_labels' not in predictions[0].extra_fields.keys():
            result_str = '\n'
            for ensemble_model in cfg.ENSEMBLE_LEARNING.TYPE:
                rel_eval_result_dict = {}
                # tradictional Recall@K
                eval_recall = SGRecall(rel_eval_result_dict)
                eval_recall.register_container(mode)
                evaluator['eval_recall_%s' % ensemble_model] = eval_recall
                ...
                evaluator['eval_nog_recall_%s' % ensemble_model] = eval_nog_recall
                ...
                evaluator['eval_zeroshot_recall_%s' % ensemble_model] = eval_zeroshot_recall
                ...
                evaluator['eval_pair_accuracy_%s' % ensemble_model] = eval_pair_accuracy
                ...
                evaluator['eval_mean_recall_%s' % ensemble_model] = eval_mean_recall

                # used for no graph constraint mean Recall@K
                eval_ng_mean_recall = SGNGMeanRecall(rel_eval_result_dict, num_rel_category, dataset.ind_to_predicates,
                                                     print_detail=True)
                eval_ng_mean_recall.register_container(mode)
                evaluator['eval_ng_mean_recall_%s' % ensemble_model] = eval_ng_mean_recall
                ...
                result_str += '=' * 100 + '\n'
                ensemble_result_dict[ensemble_model] = rel_eval_result_dict
                logger.info(result_str)
                if ensemble_model == 'beta':
        ...
        if avg_metrics is None:
            avg_metrics = np.mean(rel_eval_result_dict[mode + '_mean_recall'][100]) #cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC][100])
        if output_folder:
    ...
    return float(avg_metrics), result_dict_list_to_log  #, class_level_recall
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, ensemble_model=None):
    """
    Returns:
    ...
    if ensemble_model!= None:
        local_container['rel_scores'] = prediction.get_field(
            'pred_rel_scores_%s' % ensemble_model).detach().cpu().numpy()  # (#pred_rels, num_pred_class)
        local_container['pred_rel_inds'] = prediction.get_field('rel_pair_idxs_%s' % ensemble_model).long().detach().cpu().numpy()
    ...
    if mode != 'sgdet':
        if ensemble_model != None:
            evaluator['eval_pair_accuracy_%s' % ensemble_model].prepare_gtpair(local_container)
        else:
            evaluator['eval_pair_accuracy'].prepare_gtpair(local_container)

    # to calculate the prior label based on statistics
    if ensemble_model != None:
        evaluator['eval_zeroshot_recall_%s' % ensemble_model].prepare_zeroshot(global_container, local_container)
    else:
    ...
    if ensemble_model != None:
        local_container = evaluator['eval_recall_%s' % ensemble_model].calculate_recall(global_container,
    ...
    # No Graph Constraint
    if ensemble_model != None:
        evaluator['eval_nog_recall_%s' % ensemble_model].calculate_recall(global_container, local_container, mode)
    else:
        evaluator['eval_nog_recall'].calculate_recall(global_container, local_container, mode)
    # GT Pair Accuracy
    if ensemble_model != None:
        evaluator['eval_pair_accuracy_%s' % ensemble_model].calculate_recall(global_container, local_container, mode)
    else:
        evaluator['eval_pair_accuracy'].calculate_recall(global_container, local_container, mode)
    # Mean Recall
    if ensemble_model != None:
        evaluator['eval_mean_recall_%s' % ensemble_model].collect_mean_recall_items(global_container, local_container, mode)
    else:
    ...
    if ensemble_model != None:
        evaluator['eval_ng_mean_recall_%s' % ensemble_model].collect_mean_recall_items(global_container, local_container, mode)
    else:
        evaluator['eval_ng_mean_recall'].collect_mean_recall_items(global_container, local_container, mode)
    # Zero shot Recall
    if ensemble_model != None:
        evaluator['eval_zeroshot_recall_%s' % ensemble_model].calculate_recall(global_container, local_container, mode)
    else:
