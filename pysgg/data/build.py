...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, transform_depth=None, is_train=True):
    """
    Arguments:
    ...
    for dataset_name in dataset_list:
        ...
        if transform_depth is not None:
            args["depth_transforms"] = transform_depth
        dataset = factory(**args)
...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0):
    ...
    transforms, transform_depth = None if not is_train and cfg.TEST.BBOX_AUG.ENABLED else build_transforms(cfg,
                                                                                                           is_train)
    datasets = build_dataset(cfg, dataset_list, transforms, DatasetCatalog, transform_depth=transform_depth,
                             is_train=is_train)
