...
class RelationLossComputation(object):
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, relation_probs_norm=None, labeling_prob=None, rel_labels_one_hot_count=None):
        ...
        if self.pcpl:
            relation_logits_raw, relation_logits = relation_logits
        if self.attri_on:
        ...
        if len(torch.nonzero(rel_labels != -1)) == 0:
            ...
            #weight, loss_center, rel_features = self.pcpl_weight(relation_logits_raw, rel_labels)
            loss_relation = F.cross_entropy(relation_logits, rel_labels.long(), weight=weight)
        elif relation_probs_norm is not None:
        ...
        if self.attri_on:
            ...
            return loss_relation, loss_refine_obj, loss_center, rel_features, rel_targets
