...
from .anchor_generator_orig import generate_anchors
...
@registry.RPN_HEADS.register("DoubleConvRPNHead")
class RPNHeadDoubleConv(nn.Module):
    """
    Adds a simple RPN Head with classification and regression heads
    """
    ...
    def __init__(self, cfg, in_channels, mid_channels, num_anchors):
        ...
        Arguments:
            ...
            num_anchors (int): number of anchors to be predicted
        """
        super(RPNHeadDoubleConv, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1),
            nn.ReLU6(inplace=True),  # Tensorflow docs use Relu6, so let's use it too....
            nn.Conv2d(mid_channels, 120,  # self.anchor_target_dim * self._A,
                      kernel_size=1)
        )
        self.cls_logits = nn.Conv2d(mid_channels, num_anchors, kernel_size=1, stride=1)
        self.bbox_pred = nn.Conv2d(
...
class RPNHeadDoubleConv(nn.Module):
    ...
    def __init__(self, cfg, in_channels, mid_channels, num_anchors):
        ...
        ans_np = generate_anchors(base_size=cfg.RPN.ANCHOR_SIZES,
                                  feat_stride=cfg.RPN.ANCHOR_STRIDE[0],
