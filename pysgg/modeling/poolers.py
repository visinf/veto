...
class Pooler(nn.Module):
    ...
    def __init__(self, output_size, scales, sampling_ratio, in_channels=512, cat_all_levels=False):
        ...
        for scale in scales:
            union_poolers.append(
                ROIAlign(
                    output_size=(7, 7), spatial_scale=scale, sampling_ratio=sampling_ratio
                )
            )
        #self.pooling_size = 8
        self.poolers = nn.ModuleList(poolers)
        self.union_poolers = nn.ModuleList(union_poolers)
        #self.rel_pooler = RoIAlign((self.pooling_size, self.pooling_size), spatial_scale=1 / 16, sampling_ratio=-1)
    ...
    def forward(self, x, boxes, depth_features=None, union=False):
        """
        Arguments:
        ...
        """
        if union:
            self.poolers = self.union_poolers
        num_levels = len(self.poolers)
        rois = self.convert_to_roi_format(boxes)
        assert rois.size(0) > 0
        if num_levels == 1 and depth_features is not None:
            return self.poolers[0](x[0], rois), self.poolers[0](depth_features, rois)
        elif num_levels == 1:
        ...
        if depth_features is not None:
            final_channels_depth = depth_features.shape[1]
            if (len(self.poolers) > 1):
                level = 2
            else:
                level = 0
            per_level_feature = depth_features

            pooler = self.poolers[level]
            result_d = pooler(per_level_feature, rois).to(dtype)
        else:
            result_d = None
        ...
        if depth_features is not None:
            return result, result_d #, result_rel, result_d_rel
        else:
            return result
